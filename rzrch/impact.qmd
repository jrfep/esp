...
Esta es el total de mis publicaciones que tienen citas en algún instrumento de políticas globales, según los valores registrados por PlumX.
...
PMX_ALL |> 
  ...
  summarise(
    `Total documents`=n(),
    `Total policy citations`=sum(POLICY_CITED_BY_COUNT))|>
...
En la siguiente tabla presento una lista detallada de estas publicaciones con los enlaces a sus perfiles en PlumX.

```{r}
PMX_ALL |> 
  filter(!is.na(POLICY_CITED_BY_COUNT)) |> 
  transmute(
    DOI = sprintf("<a href='https://doi.org/%1$s'>%2$s</a>", doi, title), 
