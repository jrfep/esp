...
bib2 <- data.frame(bib1)

CRediT_folder <- "data"
credit <-read_excel(here::here(CRediT_folder, "CRediT-my-publications-2026.xlsx"), 
                    "Contributions per pub") 
dois <- unique(c(
  credit |> filter(!is.na(doi)) |> pull(doi),
  bib2 |> filter(!is.na(doi)) |> pull(doi)
))


conn <- file(here::here("data/plumx/","doi-list"), "w")
writeLines(unique(tolower(dois)),conn)
close(conn)
```
...
PMX_ALL <- map(jsonFiles, readPlumxJson) |> 
  bind_rows() |> 
  left_join(bib2,by="doi")
